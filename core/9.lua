	--[[ Focus and target spec portrait ]]--
local SpecTextures = {};
...
local FocusFrameSpec = CreateFrame("Frame"); -- Focus Frame
	FocusFrameSpec:RegisterEvent("PARTY_MEMBERS_CHANGED");
	FocusFrameSpec:RegisterEvent("PLAYER_TARGET_CHANGED");
	FocusFrameSpec:RegisterEvent("PLAYER_FOCUS_CHANGED");
	FocusFrameSpec:RegisterEvent("UNIT_FACTION");
	FocusFrameSpec:RegisterEvent("ARENA_OPPONENT_UPDATE");
	FocusFrameSpec:SetScript("OnEvent",function() 
	...
	if instanceType == "arena" then
		for i = 1, 5 do
			if UnitIsUnit("focus","arena"..i) then
				...
				if icon then
					SetPortraitToTexture(FocusFrame.portrait,icon);
					return;
				end;
			end;
		end;
	elseif instanceType == "pvp" then
		for scoreIndex = 1, GetNumBattlefieldScores() do
			...
			if GetUnitName("focus",true) == name then
...
local TargetFrameSpec = CreateFrame("Frame"); -- Target Frame
