...
function AutoLoot:LootItems(numItems)
    ...
    for i = numItems, 1, -1 do
        ...
        if locked or (quality and quality >= lootThreshold) then
            ...
            if slotType ~= LOOT_SLOT_ITEM or (not internal.isClassic and isQuestItem) or
                self:ProcessLootItem(itemLink, quantity) then
                numItems = numItems - 1;
                LootSlot(i);
            end
        end
...
local function AddMessage(...)
    _G.DEFAULT_CHAT_FRAME:AddMessage(strjoin(" ", tostringall(...)))
end
function AutoLoot:Help(msg)
    ...
    if not cmd or cmd == "" or cmd == "help" then
        ...
        AddMessage(
            "  |cff58C6FA/sal sound            -|r  |cffEEE4AEPlay a Sound when Inventory is full while looting|r");
